...
class NoiseOps_Diagonal(BaseTestNoiseLO):
    def setup_method(self):
        ...
        operator2 = brahmap.core.NoiseCovLO_Diagonal(
            size=size, input=power_spec, input_type="power_spectrum", dtype=dtype
        )
        numpy_operator1 = np.diag(covariance)

        inv_operator1 = brahmap.core.InvNoiseCovLO_Diagonal(
            size=size, input=covariance, input_type="covariance", dtype=dtype
        )
        inv_operator2 = brahmap.core.InvNoiseCovLO_Diagonal(
            size=size, input=power_spec, input_type="power_spectrum", dtype=dtype
        )
        numpy_inv_operator1 = np.diag(1.0 / covariance)
