...
# class SharedMemoryMode(IntEnum):
#     TREE_REDUCE = 1
#     SERIALIZED = 2
...
class SharedMemProcessTimeSamples(object):
    """A data container to store pre-processed pointing information,
    pre-computed map-making weights and metadata.
    ...
    def _allocate_shmem_arrays(
        ...
        self.__hit_counts, self.__win_hit_counts = mgr.alloc_shared_array(
            self.npix, dint, comm, comm_root
        )
        self.__weighted_counts, self.__win_weighted_counts = mgr.alloc_shared_array(
            self.npix, dfloat, comm, comm_root
        )

        if self.solver_type != SolverType.I:
            self.__weighted_sin_sq, self.__win_weighted_sin_sq = mgr.alloc_shared_array(
                self.npix, dfloat, comm, comm_root
            )
            self.__weighted_cos_sq, self.__win_weighted_cos_sq = mgr.alloc_shared_array(
                self.npix, dfloat, comm, comm_root
            )
            self.__weighted_sincos, self.__win_weighted_sincos = mgr.alloc_shared_array(
                self.npix, dfloat, comm, comm_root
            )

        if self.solver_type == SolverType.IQU:
            self.__weighted_sin, self.__win_weighted_sin = mgr.alloc_shared_array(
                self.npix, dfloat, comm, comm_root
            )
            self.__weighted_cos, self.__win_weighted_cos = mgr.alloc_shared_array(
                self.npix, dfloat, comm, comm_root
            )
    ...
    def _compute_weights(
        ...
        """Computes the hit counts, observed pixels, and trigonometric"""

        mgr = self.__shared_mem_manager
        dint = self.__pointings.dtype
        dfloat = self.dtype_float

        # Allocate node-level shared arrays
        ...
        # Allocate node-level shared arrays that are supposed to be filled
        # only at the end
        self.__observed_pixels, self.__win_observed_pixels = mgr.alloc_shared_array(
            self.npix, dint, mgr.node_comm, mgr.node_root
        )
        self.__old2new_pixel, self.__win_old2new_pixel = mgr.alloc_shared_array(
            self.npix, dint, mgr.node_comm, mgr.node_root
        )
        self.__pixel_flag, self.__win_pixel_flag = mgr.alloc_shared_array(
            self.npix, bool, mgr.node_comm, mgr.node_root
        )
        if self.solver_type != SolverType.I:
            (
                self.__one_over_determinant,
                self.__win_one_over_determinant,
            ) = mgr.alloc_shared_array(self.npix, dfloat, mgr.node_comm, mgr.node_root)

        # Allocate local arrays
        if self.solver_type != SolverType.I:
            self.__sin2phi = np.zeros(self.nsamples, dtype=dfloat)
            self.__cos2phi = np.zeros(self.nsamples, dtype=dfloat)
        ...
        if self.solver_type == SolverType.I:
            self.__new_npix = compute_weights_shared.compute_weights_shmem_pol_I(
                npix=self.npix,
                nsamples=self.nsamples,
                pointings=self.__pointings,
                pointings_flag=self.__pointings_flag,
                noise_weights=noise_weights,
                node_hit_counts=self.__hit_counts,
                win_hit_counts=self.__win_hit_counts,
                node_weighted_counts=self.__weighted_counts,
                win_weighted_counts=self.__win_weighted_counts,
                observed_pixels=self.__observed_pixels,
                __old2new_pixel=self.__old2new_pixel,  # type: ignore
                pixel_flag=self.__pixel_flag,
            ...
            if self.solver_type == SolverType.QU:
                compute_weights_shared.compute_weights_shmem_pol_QU(
                    ...
                    sin2phi=self.__sin2phi,
                    cos2phi=self.__cos2phi,
                    node_weighted_sin_sq=self.__weighted_sin_sq,
                    win_weighted_sin_sq=self.__win_weighted_sin_sq,
                    node_weighted_cos_sq=self.__weighted_cos_sq,
                    win_weighted_cos_sq=self.__win_weighted_cos_sq,
                    node_weighted_sincos=self.__weighted_sincos,
                    win_weighted_sincos=self.__win_weighted_sincos,
                    one_over_determinant=self.__one_over_determinant,
                    node_root=mgr.node_root,
                    tree_grp_comm=mgr.tree_grp_comm,
                    tree_grp_root_comm=mgr.tree_grp_root_comm,
                ...
                compute_weights_shared.compute_weights_shmem_pol_IQU(
                    ...
                    node_weighted_sincos=self.__weighted_sincos,
                    win_weighted_sincos=self.__win_weighted_sincos,
                    node_weighted_sin=self.__weighted_sin,
                    win_weighted_sin=self.__win_weighted_sin,
                    node_weighted_cos=self.__weighted_cos,
                    win_weighted_cos=self.__win_weighted_cos,
                    one_over_determinant=self.__one_over_determinant,
                    node_root=mgr.node_root,
                    tree_grp_comm=mgr.tree_grp_comm,
                    tree_grp_root_comm=mgr.tree_grp_root_comm,
            ...
            if mgr.node_rank == mgr.node_root:
                self.__new_npix = compute_weights_shared.get_pixel_mask_pol(
                    solver_type=self.solver_type,
                    npix=self.npix,
                    threshold=self.threshold,
                    hit_counts=self.__hit_counts,
            ...
            mgr.node_comm.Bcast([self.__new_npix, MPI.INT], root=mgr.node_root)
    ...
    def _repixelization(self):
        ...
        self.__observed_pixels, self.__win_observed_pixels = _realloc_shared(
            self.__observed_pixels,
            self.__win_observed_pixels,
            new_npix,
            dint,
        )
        self.__hit_counts, self.__win_hit_counts = _realloc_shared(
            self.__hit_counts,
            self.__win_hit_counts,
            new_npix,
            dint,
        )
        self.__weighted_counts, self.__win_weighted_counts = _realloc_shared(
            self.__weighted_counts,
            self.__win_weighted_counts,
            new_npix,
            dfloat,
        )

        if self.solver_type != SolverType.I:
            self.__weighted_sin_sq, self.__win_weighted_sin_sq = _realloc_shared(
                self.__weighted_sin_sq,
                self.__win_weighted_sin_sq,
                new_npix,
                dfloat,
            )
            self.__weighted_cos_sq, self.__win_weighted_cos_sq = _realloc_shared(
                self.__weighted_cos_sq,
                self.__win_weighted_cos_sq,
                new_npix,
                dfloat,
            )
            self.__weighted_sincos, self.__win_weighted_sincos = _realloc_shared(
                self.__weighted_sincos,
                self.__win_weighted_sincos,
                new_npix,
                dfloat,
            )
            (
        ...
        if self.solver_type == SolverType.IQU:
            self.__weighted_sin, self.__win_weighted_sin = _realloc_shared(
                self.__weighted_sin,
                self.__win_weighted_sin,
                new_npix,
                dfloat,
            )
            self.__weighted_cos, self.__win_weighted_cos = _realloc_shared(
                self.__weighted_cos,
                self.__win_weighted_cos,
                new_npix,
                dfloat,
            )

        # Maybe put fence here
        mgr.node_comm.Barrier()
