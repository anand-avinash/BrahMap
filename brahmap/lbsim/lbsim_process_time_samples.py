...
from ..core import SolverType, ProcessTimeSamples, SharedMemProcessTimeSamples
...
def _process_lbsim_observations(
    nside: int,
...
) -> tuple[List[lbs.Observation], npt.NDArray[np.int_], npt.NDArray[np.number]]:
    """Helper function to parse and process litebird_sim observations and pointings."""
    ...
    start_idx = 0
    end_idx = 0
    for obs_idx, (obs, curr_pointings) in enumerate(zip(obs_list, ptg_list)):
        if hwp is None:
            hwp_angle = None
        else:
    ...
    return obs_list, pix_indices, pol_angles
...
class LBSimProcessTimeSamples(ProcessTimeSamples):
    ...
    noise_weights : npt.NDArray[np.number] | None, optional
        ...
        default `None`. If set as `None`, inverse noise variance is set to 1
    ...
    output_coordinate_system : lbs.CoordinateSystem, optional
        ...
        by default `lbs.CoordinateSystem.Galactic`
    ...
    def __init__(
        ...
        self.__obs_list, pix_indices, pol_angles = _process_lbsim_observations(
            nside=nside,
...
class LBSimSharedMemProcessTimeSamples(SharedMemProcessTimeSamples):
    ...
    Similar to [`LBSimProcessTimeSamples`][brahmap.lbsim.LBSimProcessTimeSamples],
    this container object can be used to create pointing operators,
    ...
    noise_weights : npt.NDArray[np.number] | None, optional
        Array of noise inverse noise variance for each time sample, by
        default `None`. If set as `None`, inverse noise variance is set to 1 for each time sample
    output_coordinate_system : lbs.CoordinateSystem, optional
        The celestial coordinate system to use for the generated output maps,
        by default `lbs.CoordinateSystem.Galactic`
    threshold : float, optional
